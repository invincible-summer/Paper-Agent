...
> **论文调研智能体** — 对话组织研究，「阅研」工作台陪你在原文中阅读、追问与记录发现。支持文献检索、深读、研究地图、阅读路径与综述写作。可作为 OpenAI 兼容服务接入清小搭智能体广场。

> 本文档只记录当前版本的功能与用法，不写历史沿革。架构细节见 [docs/DESIGN.md](docs/DESIGN.md)。

## 功能

所有用户页面共享「阅研」专业工作台：暖纸底、苔绿主色和清晰的文字层级。桌面以功能导航、会话历史、对话正文、当前会话资料组成高密度布局；窄屏自动使用抽屉，入口集中在顶栏，手机开关不会覆盖桌面折叠偏好。对话（`/chat`）组织文献与研究任务，PDF 附件可进入独立的「阅研」阅读工作台；Agent 按需调度类型化工具与技能：

- **阅研 · PDF 阅读工作台** — 从当前会话的 PDF 附件或左侧导航「阅研」入口（`/reader`，列出你上传过的全部 PDF）打开原版阅读页面，支持连续滚动、单页翻阅、宽屏双页、目录、缩放和阅读位置恢复。同一篇论文从任何入口打开都复用同一个阅读会话——位置、笔记、划线与讨论跨入口保留、不同论文互不干扰。划选后可专业翻译、解释、追问、划线或记下发现；译文参考页内语境与本篇术语表，助读回答支持数学公式。阅读讨论独立保存，笔记保留原文出处和自己的判断，点击“带回对话”后可继续比较与写作，再从聊天中的链接返回原文。
- **文件中心**（`/files`）— **论文集**：跨会话汇总检索到的网络论文元数据（标题/作者/摘要/来源链接/关联会话），可按标题、作者、期刊、关键词搜索，链接跳转出版方页面；网络论文不下载全文。**个人文件**：列出你上传过的全部附件（类型/大小/时间），支持预览提取文本、下载原件（中文名保留），PDF 一键进入阅研。
...
- 中英双语 UI；「阅研纸感」设计风格（暖纸底 + 苔绿主色 + 赭石点缀）
...
1. 从聊天顶栏“资料与阅读”或右侧“工作台”进入阅读空间，选择当前会话论文，再打开阅读工作台；手机顶栏使用书本图标。没有论文时可先在聊天中选择 PDF 并点击“只上传，开始阅读”，无需发送模型消息。附件上的快捷入口仍可使用。
2. 工作台在新标签页打开，主聊天的草稿和正在生成的回答保持原位。中间是原版 PDF，左右分别是目录/标记和助读/发现，可收起面板专注阅读。
3. 选中同一页内的文字，使用浮动操作条翻译、解释、追问或划线。可开启“划选即译”；在“本篇术语表”中指定专业译法。未选文字时，输入框围绕当前页提问，也可要求总结当前论文。
4. 把原文、助读解释和自己的判断存为发现，按贡献、证据、疑问、方法分类；“带回对话”会明确添加一张来源卡，不自动再调用模型。原聊天可接着使用这些发现，来源链接可回到 PDF 对应标记。

阅读交互：文字选区在原页显示绿色虚线下划线，保存“划线”后显示实线；选文不再重复占用助读栏。点击“框选图片”可拖动圈定图片、表格或公式，框选后可标记区域、记笔记及追问，区域标记支持刷新恢复和原文回跳。按 Esc 取消选择或框选模式。区域坐标不是 OCR 识别结果，助读仅在现有论文证据或图表工具能支持时解释图像。

阅读位置、术语表、标记、笔记与讨论保存在当前账号/当前会话中；关闭窗口后重新进入可继续。同一会话的研究问答与发现发布串行协调，等待时可以取消。纯翻页、查看原件和记笔记无需 LLM/VLM。

目前支持 PDF 原版阅读，DOCX 等继续使用原有文本预览；目录来自 PDF 内置书签。跨页选文请分段处理；扫描页可浏览和按页提问，暂不提供扫描页可选 OCR 覆盖层。只有定位成功的文字显示精确高亮，页级来源会明确标注。整篇主张证据图、图表专用操作热区和双论文对照尚未上线。

前端的 `predev`/`prebuild` 会从已安装的 PDF.js 复制同版本 worker、字体、CMaps 和 WASM 到被 Git 忽略的 `frontend/public/pdfjs/`；无 CDN 依赖。使用 `pnpm dev`/`pnpm build` 或 `./start.sh` 启动。
...
- `/v1` 流式请求采用管理员可调的整轮软时限与硬时限（默认 95/105 秒，均可在 `/admin/performance` 调整，无固定数值上限，唯一约束是软 ≤ 硬 − 5 秒以为降级收尾留时间；经清小搭网关转发的请求仍受网关 120 秒总超时限制，超过 120 秒的设置只有直连 `/v1` 的调用能真正用满）：验证后先发送 role 首帧，工具期间持续发送 reasoning 进度，超时或客户端中断都会安全取消并闭合为唯一 stop 帧 + `[DONE]`；每轮只读取一次工具预算策略快照，运行时保证软时限先于硬截止触发。微小 token 会按时间/字数合帧，避免逐字符 SSE 产生数十倍协议开销。reasoning 各段之间插入显式换行分隔：模型思考与每个工具模块之间空一行，同一工具模块内的每条进度独占一行，客户端拼接 `delta.reasoning` 后仍是分段排版而不是一整段。服务器侧诊断可运行 `scripts/diagnose_openai_agent_stream.py`
...
- **用户反馈**：左侧导航「意见反馈」进入 `/feedback` 页面，选择类型（问题报告 / 功能建议 / 其他）、填写内容与可选联系方式后提交（登录用户、游客与本地模式均可，同一提交者 60 秒冷却防刷）。反馈存于 `data/users.db`，**仅管理员**可在 `/admin/feedback` 按待处理/已处理筛选查看、标记处理状态或删除
...
- Node.js 22.13+ 和 pnpm（PDF.js 阅读器所需；当前开发使用 Node 22）
