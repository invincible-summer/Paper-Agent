...
SOURCE_IDS = (
    "openalex", "semantic_scholar", "arxiv", "crossref", "europepmc",
    "doaj", "hal", "openaire", "core",
)
...
def _seed() -> PaperSearchPolicy:
    try:
        ...
        sources = {name: bool(raw_sources.get(name, True)) for name in SOURCE_IDS}
        ...
        return PaperSearchPolicy(sources={name: True for name in SOURCE_IDS})
...
def _normalize_sources(value: dict | None, *, base: dict[str, bool] | None = None) -> dict[str, bool]:
    result = dict(base or {name: True for name in SOURCE_IDS})
    if value is None:
        return {name: bool(result.get(name, True)) for name in SOURCE_IDS}
...
def _row_to_policy(row) -> PaperSearchPolicy:
    ...
    return PaperSearchPolicy(
        ...
        version=int(row[7]), updated_by=str(row[8] or "bootstrap"),
...
def get_paper_search_policy() -> PaperSearchPolicy:
    ...
    try:
        row = conn.execute(
            ...
            "fetch_policy_disclosure, version, updated_by, updated_at "
            "FROM paper_search_policy WHERE id = 1"
        ).fetchone()
        if row is None:
            seed = _seed()
            conn.execute(
                "INSERT INTO paper_search_policy VALUES (1, ?, ?, ?, ?, ?, ?, ?, 1, 'bootstrap', ?)",
                (json.dumps(seed.sources, sort_keys=True), seed.search_deadline_seconds,
                 ...
                 seed.fetch_policy_disclosure, now),
            )
            conn.commit()
            row = conn.execute(
                "SELECT sources_json, search_deadline_seconds, per_source_timeout_seconds, "
...
def _validate(policy: PaperSearchPolicy) -> None:
    if not 10 <= policy.search_deadline_seconds <= 120:
        raise PaperSearchSettingsError("检索总时限必须在 10–120 秒之间")
    ...
    if not 0 <= policy.fulltext_verify_timeout_seconds <= 120:
        raise PaperSearchSettingsError("全文探测总时限必须在 0–120 秒之间")
    ...
    try:
        cursor = conn.execute(
            """UPDATE paper_search_policy SET sources_json = ?,
                   ...
                   paper_fetch_mode = ?, fetch_policy_disclosure = ?,
            ...
            (json.dumps(candidate.sources, sort_keys=True), candidate.search_deadline_seconds,
             ...
             candidate.fetch_policy_disclosure, updated_by, candidate.updated_at,
