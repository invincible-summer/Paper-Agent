      ...
      <div className="w-full max-w-sm rounded-2xl border border-border-light bg-surface p-8" style={{ boxShadow: "var(--shadow-md)" }}>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete={mode === "login" ? "current-password" : "new-password"} className="w-full rounded-lg border border-border-light bg-bg px-3 py-2 text-sm text-fg outline-none transition-colors focus:border-accent/50" placeholder={mode === "register" ? "至少 8 位" : ""} />
          </div>
